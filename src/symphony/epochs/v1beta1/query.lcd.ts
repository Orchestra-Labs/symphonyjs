...
export class LCDQueryClient {
  ...
  async epochInfos(
    _params: QueryEpochsInfoRequest = {},
  ): Promise<QueryEpochsInfoResponseSDKType> {
    const endpoint = `symphony/epochs/v1beta1/epochs`;
    return await this.req.get<QueryEpochsInfoResponseSDKType>(endpoint);
  }
  /* CurrentEpoch provide current epoch of specified identifier */
  async currentEpoch(
    params: QueryCurrentEpochRequest,
  ): Promise<QueryCurrentEpochResponseSDKType> {
    const options: any = {
      params: {},
    };
    ...
    const endpoint = `symphony/epochs/v1beta1/current_epoch`;
    return await this.req.get<QueryCurrentEpochResponseSDKType>(
      endpoint,
      options,
    );
  }
