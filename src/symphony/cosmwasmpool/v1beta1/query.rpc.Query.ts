...
export class QueryClientImpl implements Query {
  ...
  pools(request: PoolsRequest = {
    ...
    const promise = this.rpc.request("symphony.cosmwasmpool.v1beta1.Query", "Pools", data);
    return promise.then(data => PoolsResponse.decode(new BinaryReader(data)));
  }
  params(request: ParamsRequest = {}): Promise<ParamsResponse> {
    const data = ParamsRequest.encode(request).finish();
    const promise = this.rpc.request("symphony.cosmwasmpool.v1beta1.Query", "Params", data);
    return promise.then(data => ParamsResponse.decode(new BinaryReader(data)));
  }
  contractInfoByPoolId(request: ContractInfoByPoolIdRequest): Promise<ContractInfoByPoolIdResponse> {
    const data = ContractInfoByPoolIdRequest.encode(request).finish();
    const promise = this.rpc.request("symphony.cosmwasmpool.v1beta1.Query", "ContractInfoByPoolId", data);
