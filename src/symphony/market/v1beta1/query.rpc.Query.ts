...
export interface Query {
  ...
  exchangeRequirements(
    request?: QueryExchangeRequirementsRequest,
  ): Promise<QueryExchangeRequirementsResponse>;
}
export class QueryClientImpl implements Query {
  ...
  swap(request: QuerySwapRequest): Promise<QuerySwapResponse> {
    ...
    const promise = this.rpc.request(
      'symphony.market.v1beta1.Query',
      'Swap',
      data,
    );
    return promise.then(data =>
      QuerySwapResponse.decode(new BinaryReader(data)),
    );
  }
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
  ...
  exchangeRequirements(
    ...
    return promise.then(data =>
      QueryExchangeRequirementsResponse.decode(new BinaryReader(data)),
    );
  }
}
