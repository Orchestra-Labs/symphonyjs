...
export interface Msg {
  ...
  inboundTransfer(
    request: MsgInboundTransfer,
  ): Promise<MsgInboundTransferResponse>;
  /** OutboundTransfer is used for outbound transfers (OSMO -> <other_chain>). */
  outboundTransfer(
    request: MsgOutboundTransfer,
  ): Promise<MsgOutboundTransferResponse>;
  /** UpdateParams is used for updating module params. */
  updateParams(request: MsgUpdateParams): Promise<MsgUpdateParamsResponse>;
  /** ChangeAssetStatus will change the provided asset's status. */
  changeAssetStatus(
    request: MsgChangeAssetStatus,
  ): Promise<MsgChangeAssetStatusResponse>;
}
export class MsgClientImpl implements Msg {
  ...
  inboundTransfer(
    ...
    const promise = this.rpc.request(
      'symphony.bridge.v1beta1.Msg',
      'InboundTransfer',
      data,
    );
    return promise.then(data =>
      MsgInboundTransferResponse.decode(new BinaryReader(data)),
    );
  }
  outboundTransfer(
    request: MsgOutboundTransfer,
    ...
    return promise.then(data =>
      MsgOutboundTransferResponse.decode(new BinaryReader(data)),
    );
  }
  updateParams(request: MsgUpdateParams): Promise<MsgUpdateParamsResponse> {
  ...
  changeAssetStatus(
    ...
    return promise.then(data =>
      MsgChangeAssetStatusResponse.decode(new BinaryReader(data)),
    );
  }
}
