...
export class LCDQueryClient {
  ...
  async getTxsEvent(
    params: GetTxsEventRequest,
  ): Promise<GetTxsEventResponseSDKType> {
    const options: any = {
      params: {},
    };
  ...
  async getBlockWithTxs(
    params: GetBlockWithTxsRequest,
  ): Promise<GetBlockWithTxsResponseSDKType> {
    const options: any = {
      params: {},
    };
    if (typeof params?.pagination !== 'undefined') {
    ...
    return await this.req.get<GetBlockWithTxsResponseSDKType>(
      endpoint,
      options,
    );
  }
