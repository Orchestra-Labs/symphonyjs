...
export class LCDQueryClient {
  ...
  ): Promise<QueryCurrentPlanResponseSDKType> {
  ...
  async appliedPlan(
    params: QueryAppliedPlanRequest,
  ): Promise<QueryAppliedPlanResponseSDKType> {
  ...
  ): Promise<QueryUpgradedConsensusStateResponseSDKType> {
    const endpoint = `cosmos/upgrade/v1beta1/upgraded_consensus_state/${params.lastHeight}`;
    return await this.req.get<QueryUpgradedConsensusStateResponseSDKType>(
      endpoint,
    );
  }
  ...
  async moduleVersions(
  ...
  ): Promise<QueryModuleVersionsResponseSDKType> {
    const options: any = {
      params: {},
    };
    ...
    return await this.req.get<QueryModuleVersionsResponseSDKType>(
      endpoint,
      options,
    );
  }
  ...
  async authority(
    _params: QueryAuthorityRequest = {},
  ): Promise<QueryAuthorityResponseSDKType> {
